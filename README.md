  ...
  - [Documentation](#documentation)
    ...
    - [smartini.**type\_converters**](#smartinitype_converters)
      - [smartini.type\_converters.**converters**](#smartinitype_convertersconverters)
        ...
        - [smartini.type\_converters.converters.**converter**](#smartinitype_convertersconvertersconverter)
        ...
        - [smartini.type\_converters.converters.**DEFAULT\_STRING\_CONVERTER**](#smartinitype_convertersconvertersdefault_string_converter)
...
2. Create a custom converter that converts a value to a `datetime.time` object.
    ...
    ```python
    from smartini.type_converters.converters import converter, WrongType
    from datetime import time

    @converter
    def time_converter(string:str) -> time:
        try:
...
- **strip_option_values** (`bool`, optional)

    Whether to strip leading and trailing whitespace from option values. Defaults to `True`.
...
##### smartini.type_converters.converters.**converter**

```python
type_converters.converter(processor)
```

Decorator to create a new [TypeConverter](#smartinitype_convertersconverterstypeconverter).
...
##### smartini.type_converters.converters.**DEFAULT_STRING_CONVERTER**

```python
DEFAULT_STRING_CONVERTER = smartini.type_converters.string_converter()
```

[String converter](#smartinitype_convertersconvertersstring_converter) with default conversion parameters.
...
- **\*types** (`type`)
    ...
    The types to guess. If not provided, will guess all of [`ConvertibleTypes`](#smartinitype_convertersconvertersconvertibletypes).

- **fallback** ([`TypeConverter`](#smartinitype_convertersconverterstypeconverter))

    Fallback converter if no type could be guessed. Defaults to [`DEFAULT_STRING_CONVERTER`](#smartinitype_convertersconvertersdefault_string_converter).
...
- **item_converter** (`TypeConverter[Any]`)
...
- `TypeConverter[int | float | complex]`

    The numeric type converter.

##### smartini.type_converters.converters.**string_converter**

```python
type_converters.string_converter(strip_whitespace = True)
```

Create a new `string` type converter.

**Args**

- **strip_whitespace** (`bool`, optional): Whether to strip leading and trailing whitespace from the string. Defaults to `True`.
